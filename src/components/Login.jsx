...
const mobFeaturesSecureGap = ${mobFeaturesSecureGap};`;
  ...
  return (
    <div className="l-root">
      ...
      {/* ── CONFIGURADOR INTERACTIVO DE FONDO ── */}
      <button 
        type="button" 
        className="l-cal-btn" 
        onClick={() => setShowCalibrator(v => !v)}
        title="Calibrador de Fondo"
      >
        <Sliders size={18} />
      </button>

      {showCalibrator && (
        <div className="l-cal-panel">
          <div className="l-cal-header">
            <h3><Sliders size={15} /> Calibrador</h3>
            <button type="button" className="l-cal-close" onClick={() => setShowCalibrator(false)}>
              <X size={15} />
            </button>
          </div>

          <div className="l-cal-device">
            {activeDevice === 'desktop' && <><Monitor size={12} /> Escritorio / Laptop</>}
            {activeDevice === 'tablet' && <><TabletIcon size={12} /> Tablet (iPad)</>}
            {activeDevice === 'mobile' && <><Phone size={12} /> Teléfono (Móvil)</>}
          </div>

          <div className="l-cal-section">
            {/* CONTROLES DE ESCRITORIO */}
            {activeDevice === 'desktop' && (
              <>
                <div className="l-cal-row">
                  <label className="l-cal-toggle">
                    <input 
                      type="checkbox" 
                      checked={bgZoomMode === 'cover'} 
                      onChange={e => setBgZoomMode(e.target.checked ? 'cover' : 'manual')} 
                    />
                    Ajuste automático (cover)
                  </label>
                </div>

                {bgZoomMode === 'manual' && (
                  <div className="l-cal-row">
                    <div className="l-cal-label-row">
                      <span>Zoom / Tamaño</span>
                      <span className="val">{bgZoom}%</span>
                    </div>
                    <input 
                      type="range" min="50" max="250" value={bgZoom} 
                      className="l-cal-slider" onChange={e => setBgZoom(Number(e.target.value))} 
                    />
                  </div>
                )}
                ...
                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    ...
                    <span className="val">{bgX}%</span>
                  </div>
                  <input 
                    type="range" min="0" max="100" value={bgX} 
                    className="l-cal-slider" onChange={e => setBgX(Number(e.target.value))} 
                  />
                </div>
                ...
                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    ...
                    <span className="val">{bgY}%</span>
                  </div>
                  <input 
                    type="range" min="0" max="100" value={bgY} 
                    className="l-cal-slider" onChange={e => setBgY(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Opacidad Oscuridad</span>
                    <span className="val">{bgDarkness}%</span>
                  </div>
                  <input 
                    type="range" min="0" max="100" value={bgDarkness} 
                    className="l-cal-slider" onChange={e => setBgDarkness(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Ancho de Foto (%)</span>
                    <span className="val">{photoWidth}%</span>
                  </div>
                  <input 
                    type="range" min="30" max="70" value={photoWidth} 
                    className="l-cal-slider" onChange={e => setPhotoWidth(Number(e.target.value))} 
                  />
                </div>
              </>
            )}
              ...
              <>
                <div className="l-cal-row">
                  <label className="l-cal-toggle">
                    <input 
                      type="checkbox" 
                      checked={tabBgZoomMode === 'cover'} 
                      onChange={e => setTabBgZoomMode(e.target.checked ? 'cover' : 'manual')} 
                    />
                    Ajuste automático (cover)
                  </label>
                </div>

                {tabBgZoomMode === 'manual' && (
                  <div className="l-cal-row">
                    <div className="l-cal-label-row">
                      <span>Zoom / Tamaño</span>
                      <span className="val">{tabBgZoom}%</span>
                    </div>
                    <input 
                      type="range" min="50" max="250" value={tabBgZoom} 
                      className="l-cal-slider" onChange={e => setTabBgZoom(Number(e.target.value))} 
                    />
                  </div>
                )}
                ...
                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Posición X (Horizontal)</span>
                    <span className="val">{tabBgX}%</span>
                  </div>
                  <input 
                    type="range" min="0" max="100" value={tabBgX} 
                    className="l-cal-slider" onChange={e => setTabBgX(Number(e.target.value))} 
                  />
                </div>
                ...
                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Posición Y (Vertical)</span>
                    <span className="val">{tabBgY}%</span>
                  </div>
                  <input 
                    type="range" min="0" max="100" value={tabBgY} 
                    className="l-cal-slider" onChange={e => setTabBgY(Number(e.target.value))} 
                  />
                </div>
              </>
            )}
              ...
              <>
                <div className="l-cal-row">
                  <label className="l-cal-toggle">
                    <input 
                      type="checkbox" 
                      checked={mobBgZoomMode === 'cover'} 
                      onChange={e => setMobBgZoomMode(e.target.checked ? 'cover' : 'manual')} 
                    />
                    Ajuste automático de Foto (cover)
                  </label>
                </div>

                {mobBgZoomMode === 'manual' && (
                  <div className="l-cal-row">
                    <div className="l-cal-label-row">
                      <span>Zoom / Tamaño de Foto</span>
                      <span className="val">{mobBgZoom}%</span>
                    </div>
                    <input 
                      type="range" min="50" max="250" value={mobBgZoom} 
                      className="l-cal-slider" onChange={e => setMobBgZoom(Number(e.target.value))} 
                    />
                  </div>
                )}
                ...
                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Posición X Foto (Horizontal)</span>
                    <span className="val">{mobBgX}%</span>
                  </div>
                  <input 
                    type="range" min="0" max="100" value={mobBgX} 
                    className="l-cal-slider" onChange={e => setMobBgX(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Posición Y Foto (Vertical)</span>
                    <span className="val">{mobBgY}%</span>
                  </div>
                  <input 
                    type="range" min="0" max="100" value={mobBgY} 
                    className="l-cal-slider" onChange={e => setMobBgY(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Altura de la Foto (%)</span>
                    <span className="val">{mobileSplit}%</span>
                  </div>
                  <input 
                    type="range" min="30" max="70" value={mobileSplit} 
                    className="l-cal-slider" onChange={e => setMobileSplit(Number(e.target.value))} 
                  />
                </div>

                <hr style={{ border: 'none', borderTop: '1px dashed rgba(212,175,55,0.2)', margin: '14px 0' }} />
                
                <div style={{ fontSize: '11px', fontWeight: 'bold', color: '#d4af37', textTransform: 'uppercase', marginBottom: '8px', letterSpacing: '0.5px' }}>
                  Ajustes de Interlineado y Spacing
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Tamaño del Logo</span>
                    <span className="val">{mobLogoSize}px</span>
                  </div>
                  <input 
                    type="range" min="80" max="220" value={mobLogoSize} 
                    className="l-cal-slider" onChange={e => setMobLogoSize(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Espacio Logo a Título</span>
                    <span className="val">{mobLogoGap}px</span>
                  </div>
                  <input 
                    type="range" min="0" max="80" value={mobLogoGap} 
                    className="l-cal-slider" onChange={e => setMobLogoGap(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Espacio Título a Subtítulo</span>
                    <span className="val">{mobTitleGap}px</span>
                  </div>
                  <input 
                    type="range" min="0" max="50" value={mobTitleGap} 
                    className="l-cal-slider" onChange={e => setMobTitleGap(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Espacio inferior Cabecera</span>
                    <span className="val">{mobHeaderPadding}px</span>
                  </div>
                  <input 
                    type="range" min="0" max="60" value={mobHeaderPadding} 
                    className="l-cal-slider" onChange={e => setMobHeaderPadding(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Espaciado interno Tarjeta</span>
                    <span className="val">{mobCardPadding}px</span>
                  </div>
                  <input 
                    type="range" min="10" max="60" value={mobCardPadding} 
                    className="l-cal-slider" onChange={e => setMobCardPadding(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Separación Campos de Entrada</span>
                    <span className="val">{mobFieldsGap}px</span>
                  </div>
                  <input 
                    type="range" min="5" max="50" value={mobFieldsGap} 
                    className="l-cal-slider" onChange={e => setMobFieldsGap(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Separación superior Formulario</span>
                    <span className="val">{mobBottomPaddingTop}px</span>
                  </div>
                  <input 
                    type="range" min="0" max="80" value={mobBottomPaddingTop} 
                    className="l-cal-slider" onChange={e => setMobBottomPaddingTop(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Separación de Bloques General</span>
                    <span className="val">{mobBottomGap}px</span>
                  </div>
                  <input 
                    type="range" min="10" max="80" value={mobBottomGap} 
                    className="l-cal-slider" onChange={e => setMobBottomGap(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Espacio Tarjeta a Características</span>
                    <span className="val">{mobCardFeaturesGap}px</span>
                  </div>
                  <input 
                    type="range" min="0" max="80" value={mobCardFeaturesGap} 
                    className="l-cal-slider" onChange={e => setMobCardFeaturesGap(Number(e.target.value))} 
                  />
                </div>

                <div className="l-cal-row">
                  <div className="l-cal-label-row">
                    <span>Espacio Características a Nota</span>
                    <span className="val">{mobFeaturesSecureGap}px</span>
                  </div>
                  <input 
                    type="range" min="0" max="80" value={mobFeaturesSecureGap} 
                    className="l-cal-slider" onChange={e => setMobFeaturesSecureGap(Number(e.target.value))} 
                  />
                </div>
              </>
            )}

            <button type="button" className="l-cal-btn-copy" onClick={handleCopyConfig}>
              {copied ? <><Check size={14} /> ¡Copiado!</> : <><Copy size={14} /> Copiar Variables JS</>}
            </button>
          </div>
        </div>
      )}
