...
// =====================
// 6. Add Recordatorio UI (OUTSIDE of the flex container)
// =====================
const targetDivMatch = content.match(/<div style=\{\{\s*display:\s*'flex',\s*justifyContent:\s*'space-between',\s*alignItems:\s*'baseline',\s*marginTop:\s*'12px'\s*\}\}>\s*\r?\n\s*<span style=\{\{ fontSize: isMobile \? '13px' : '16px', fontWeight: '900' \}\}>TOTAL A PAGAR<\/span>/);

if (!targetDivMatch) {
  console.log("Error: could not find the TOTAL A PAGAR div to inject UI.");
} else {
...
`;
  ...
  content = content.replace(targetDivMatch[0], newCheckoutSections + "\n" + targetDivMatch[0]);
}

// =====================
// 7. Change USD to EUR & Currency symbols
// =====================
...
content = content.replace(/-\\\$\{couponDiscount/g, '-€${couponDiscount');
