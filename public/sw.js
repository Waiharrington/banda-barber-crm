...
function saveOfflineRequest(requestData) {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('panda_client_offline_db', 1);
...
self.addEventListener('sync', event => {
  if (event.tag === 'sync-client-offline') event.waitUntil(replayOfflineRequests());
});
...
async function replayOfflineRequests() {
  ...
  for (const req of requests) {
    try {
      ...
      console.error('[Client SW Sync] Error:', e);
      break;
    }
  }
}
...
self.addEventListener('push', event => {
  let data = { title: 'Panda Barber Studio', body: 'Nueva notificación.' };
  if (event.data) {
    try { data = event.data.json(); } catch (e) { data = { title: 'Panda Barber Studio', body: event.data.text() }; }
  }
  ...
  event.waitUntil(
    self.registration.showNotification(data.title, {
      ...
      actions: [
        { action: 'book', title: 'Reservar Cita', icon: '/favicon.svg' },
